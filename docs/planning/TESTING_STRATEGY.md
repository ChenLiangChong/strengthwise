...
> 為 Google Play 上架做準備的測試計劃

**最後更新**：2026-01-20
**目標版本**：v4.1
**測試進度**：**~1180 個測試** ✅ (含 370 個 Service 測試)
**Monkey 測試**：5000 事件無崩潰 ✅
...
| 優先級 | 類別 | 內容 | 狀態 |
|--------|------|------|------|
| **P13** | Google Play 合規 | 隱私政策、帳號刪除、API 34 | ⚡ 手動 |
| **P14** | 設備兼容性 | Android 8/12/13/14 測試 | ⚡ 手動 |
| **P15** | 發布打包 | APK 簽名、64 位 | ⚡ 手動 |
| **P18** | 穩定性 | Monkey、ANR、弱網 | ✅ Monkey 通過 |
| **P19** | 第三方 SDK | Google Sign-In 流程 | ⚡ 手動 |
| **P20** | RLS 安全 | Supabase 權限驗證 | ⚡ 手動 |

### 需環境/工具（可選）

| 優先級 | 類別 | 說明 | 狀態 |
|--------|------|------|------|
| **P5** | 整合測試 | 需完整 Supabase 環境 | ⏳ |
| **P9** | Edge Functions | 需部署環境 | ⏳ |
| **P11** | 效能測試 | 需 benchmark 工具 | ⏳ |
| **P21** | Patrol 自動化 | 需 Patrol 設定 | ⏳ |
...
✅ 隱私政策連結有效
✅ 帳號刪除功能可用
✅ targetSdkVersion >= 34      ← 看 build.gradle.kts 的 targetSdk = 35
✅ 64 位架構支援              ← 看 defaultConfig { ndk { abiFilters } } 包含 arm64-v8a
✅ APK 簽名正確               ← flutter build appbundle 成功就是

第三方 SDK
✅ Google Sign-In 流程正常
✅ Supabase 連線正常          ← App 能登入、讀寫資料就是
✅ FCM 推播正常               ← 寄測試通知到手機有收到就是

設備兼容性
✅ Android 8.0 可運行         ← 用模擬器或實機測試 minSdk = 24
✅ Android 14 可運行          ← 用實機測試（你現在的手機）
□ 小螢幕 OK                  ← 用小螢幕模擬器
□ 平板 OK                    ← 用平板模擬器
```
...
| # | 測試案例 |
|---|----------|
| 390 | 隱私政策連結可點擊且有效 |測試完成
| 391 | 資料收集聲明完整（收集哪些、用途）| 測試完成
| 392 | 帳號刪除功能可用（Google Play 強制要求）| 測試完成
| 393 | 刪除帳號後資料完全清除 | 測試完成
| 394 | 敏感權限使用說明（相機、儲存）| 測試完成

### 應用商店要求（5 個）

| # | 測試案例 |
|---|----------|
| 395 | 目標 API Level ≥ 34（2024 要求）| 測試完成
| 396 | 年齡分級內容符合宣告 | 測試完成
| 397 | 無違規廣告/內購行為 | 測試完成
| 398 | App 名稱/圖示/截圖符合規範 | 測試完成
| 399 | 功能說明與實際功能一致 | 測試完成
...
| # | 流程 |
|---|------|
| 276 | 學員選時段 → 確認 → requested | 測試完成
| 277 | 教練確認 → confirmed → 自動創建 | 測試完成
| 278 | 教練拒絕 → rejected | 測試完成
| 279 | 任一方取消 → cancelled | 測試完成
| 280 | 臨時課程 → confirmed |
| 281 | 一鍵續約 → 預填資料 |
