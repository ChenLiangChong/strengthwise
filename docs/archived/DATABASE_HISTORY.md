...
### v5.2: 邀請碼 RLS 安全修復（2026-02-09）

**問題**：任何已認證用戶可刪除任意邀請碼
- DELETE 政策使用 `expires_at > now()` 而非 `coach_id = auth.uid()`

**解決方案**：
- 新增 `migrations/35_security_fixes.sql`
- DELETE 政策改為 `auth.uid() = coach_id`（僅教練可刪自己的）

**影響**：invite_codes 表 RLS 政策
