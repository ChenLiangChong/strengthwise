...
### ✅ 已完成：專業級統計系統（2024-12-23）
**目標**：讓用戶看到訓練成果，提高使用黏著度

**基礎統計功能**：
- [x] 訓練頻率統計（本週/本月次數、連續天數）
- [x] 訓練量趨勢圖表（美化折線圖，漸層填充）
- [x] 身體部位分布統計（進度條顯示）
- [x] 個人最佳記錄（PR）- 按身體部位分類，每個部位 Top 1
- [x] 訓練建議系統
- [x] 多時間範圍切換（本週/本月/三個月/本年）
- [x] 即時數據更新（停用快取）

**專業統計功能**：
- [x] **力量進步追蹤**：每個動作的重量曲線、1RM 估算、PR 標記
- [x] **肌群平衡分析**：推/拉/腿比例、不平衡警告、訓練建議
- [x] **訓練日曆熱力圖**：GitHub 風格、連續訓練天數、強度可視化
- [x] **完成率統計**：弱點動作識別、效率分析

**UI 改進**：
- [x] 5-Tab 專業架構（概覽/力量進步/肌群平衡/訓練日曆/完成率）
- [x] 力量進步按身體部位分類（可展開/收合）
- [x] 圖表美化（漸層、圓點、觸摸提示）
- [x] 顏色編碼系統（各身體部位專屬顏色）
- [x] 個人記錄卡片優化（各部位 Top 1，NEW 標籤）

**實作細節**：
- 使用 `fl_chart` 圖表庫（v1.1.1）
- 動態查詢動作分類
- 智能數據解析（支援舊/新格式）
- 9 個新統計模型類
- 4 個核心服務方法（~500 行代碼）
...
**待完成工作**：
- [ ] **力量進步收藏功能**：使用者可以標記喜歡的運動，優先顯示在力量進步頁面
- [ ] **力量進步選擇頁面**：如果沒有收藏的運動，顯示類似選擇訓練類型的頁面
  - 採用 5 層分類導航（訓練類型 → 身體部位 → 特定肌群 → 器材類別 → 動作本身）
  - 只顯示使用者有完成過且記錄的運動
  - 支援從此頁面添加收藏
