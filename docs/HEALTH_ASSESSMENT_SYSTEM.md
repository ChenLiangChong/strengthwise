# StrengthWise - 健康評估系統完整文檔

> 基於國際 PAR-Q+ 標準的健康篩檢與評估系統
>
> **狀態**：✅ v2.8 + v2.8.1 完成（2026-01-05）  
> **完成度**：100%
...
3. [資料庫設計](#資料庫設計)
4. [資料模型](#資料模型)
5. [教練評估備註系統](#教練評估備註系統)（⭐ v2.8.1 新增）
6. [Service 層](#service-層)
7. [UI 組件](#ui-組件)
8. [權限控制 (RLS)](#權限控制-rls)
9. [使用流程](#使用流程)
10. [相關文檔](#相關文檔)
...
1. **安全篩檢**：7 題 PAR-Q+ 基礎問卷，自動判定風險等級（低/中/高）
2. **傷病記錄**：結構化記錄學員傷病史（部位、狀態、限制）
3. **生活型態**：訓練經驗、職業活動度、可用器材
4. **訓練目標**：主要目標、數值、期望時程
5. **緊急聯絡人**：安全保障資訊
6. **教練顯示偏好**：自訂學員詳情頁顯示欄位（11 個可配置欄位）
7. **共享文檔**：學員可在「學員中心」查看並自填健康評估
8. **教練私有備註**：每位教練可對學員評估添加獨立備註（互不干擾）⭐ v2.8.1

### 技術亮點

- ✅ **自動風險評估**：三級制（低/中/高）+ 免責聲明
- ✅ **版本控制**：支援歷史評估記錄（`is_current` 標記）
- ✅ **JSONB 彈性**：進階評估使用 JSONB 儲存，方便擴充
- ✅ **GIN 索引**：支援快速查詢特定傷病或風險因子
- ✅ **RLS 安全**：教練僅能存取 `active` 學員的評估，備註完全隔離
- ✅ **共享架構**：一份評估，多位教練共用，各自獨立備註
...
- [x] 教練顯示偏好（11 個可配置欄位）
- [x] 學員共享文檔（學員中心 Tab）
- [x] 教練私有備註系統（獨立表 + RLS 隔離）⭐ v2.8.1

⏳ **未來規劃**：
- [ ] 歷史評估檢視（版本控制）
- [ ] 警示學員列表（未通過篩檢）
- [ ] 動態課前狀態（Daily Readiness）
