# 📚 StrengthWise 文檔中心

> 專案文檔索引

**最後更新**：2026-02-10

---

## 🚀 核心文檔

| 文檔 | 說明 |
|------|------|
| [DEVELOPMENT_STATUS.md](DEVELOPMENT_STATUS.md) | 📊 開發狀態、待測試項目、下一步計劃 ⭐ |
| [PROJECT_OVERVIEW.md](PROJECT_OVERVIEW.md) | 🏗️ 專案架構、技術棧、目錄結構 |
| [DATABASE_SUPABASE.md](DATABASE_SUPABASE.md) | 🗄️ 資料庫設計、RLS 策略、查詢規範 |

---

## 📖 功能文檔

| 文檔 | 說明 |
|------|------|
| [HEALTH_ASSESSMENT_SYSTEM.md](HEALTH_ASSESSMENT_SYSTEM.md) | PAR-Q+ 健康評估系統 |
| [SAAS_PLATFORM_ROADMAP.md](SAAS_PLATFORM_ROADMAP.md) | SaaS 平台路線圖 |
| [DATETIME_UTILS_GUIDE.md](DATETIME_UTILS_GUIDE.md) | 時間處理工具 API |

---

## 🎨 設計與部署

| 文檔 | 說明 |
|------|------|
| [UI_DESIGN_SYSTEM.md](UI_DESIGN_SYSTEM.md) | UI 設計系統（理念與決策）|
| [UI_DEVELOPER_GUIDE.md](UI_DEVELOPER_GUIDE.md) | UI 開發指南（快速參考）|
| [DEPLOYMENT_GUIDE.md](DEPLOYMENT_GUIDE.md) | 部署指南（APK、OAuth）|

---

## 📝 規劃文檔

### 活躍規劃（開發中）

| 文檔 | 說明 |
|------|------|
| [planning/SYNC_ARCHITECTURE_SPEC.md](planning/SYNC_ARCHITECTURE_SPEC.md) | v4.0 同步架構規格（EventBus/Realtime/FCM）⭐ |
| [planning/HOME_BOOKING_UX_SPEC.md](planning/HOME_BOOKING_UX_SPEC.md) | v3.1-B 首頁 + 行事曆 UX |
| [planning/TESTING_STRATEGY.md](planning/TESTING_STRATEGY.md) | 測試策略（564 個通過）|
| [planning/PRODUCTION_LAUNCH_GUIDE.md](planning/PRODUCTION_LAUNCH_GUIDE.md) | 生產環境發布指南 |

### 已歸檔（實施完成）

| 文檔 | 說明 |
|------|------|
| [planning/archived/EXERCISE_CLASSIFICATION_ANALYSIS.md](planning/archived/EXERCISE_CLASSIFICATION_ANALYSIS.md) | v5.0 動作分類系統分析（775 筆審核完成）✅ |
| [planning/archived/V5_IMPACT_ANALYSIS.md](planning/archived/V5_IMPACT_ANALYSIS.md) | v5.0 全面影響分析 + 向後兼容架構 ✅ |
| [planning/archived/TRACKING_MODE_SPEC.md](planning/archived/TRACKING_MODE_SPEC.md) | v3.2-v3.3 TrackingMode 擴充 ✅ |
| [planning/archived/DATA_FLOW_ANALYSIS.md](planning/archived/DATA_FLOW_ANALYSIS.md) | v3.1-E 數據流分析 + 快取策略 ✅ |
| [planning/archived/SESSION_MODE_SPEC.md](planning/archived/SESSION_MODE_SPEC.md) | v3.1-SM 教練上課模式 ✅ |
| [planning/archived/TRAINING_PERMISSION_MATRIX.md](planning/archived/TRAINING_PERMISSION_MATRIX.md) | v3.1 訓練權限矩陣 ✅ |
| [planning/archived/BOOKING_SYSTEM_OPTIMIZATION.md](planning/archived/BOOKING_SYSTEM_OPTIMIZATION.md) | v3.0 預約系統優化 ✅ |
| [planning/archived/RESPONSIVE_ARCHITECTURE_DESIGN.md](planning/archived/RESPONSIVE_ARCHITECTURE_DESIGN.md) | v3.0 響應式架構 ✅ |
| [planning/archived/COACH_PROFILE_SPEC.md](planning/archived/COACH_PROFILE_SPEC.md) | v2.9 教練公開檔案 ✅ |

---

## 🛠️ 工具與歷史

| 文檔 | 說明 |
|------|------|
| [../scripts/README.md](../scripts/README.md) | Python 工具腳本 |
| [../migrations/README.md](../migrations/README.md) | 資料庫 Migration |
| [FCM_SETUP_GUIDE.md](FCM_SETUP_GUIDE.md) | FCM 推播通知配置 ⭐ v3.0-C |
| [archived/VERSION_HISTORY.md](archived/VERSION_HISTORY.md) | 版本歷史 |
| [archived/DATABASE_HISTORY.md](archived/DATABASE_HISTORY.md) | 資料庫變更歷史 |
| [archived/SAAS_HISTORY.md](archived/SAAS_HISTORY.md) | SaaS 開發歷史 |

---

## 🔍 快速查找

| 我想... | 查看 |
|---------|------|
| 了解當前進度 | `DEVELOPMENT_STATUS.md` |
| 了解技術架構 | `PROJECT_OVERVIEW.md` |
| 查資料庫結構 | `DATABASE_SUPABASE.md` |
| 查 UI 規範 | `UI_DEVELOPER_GUIDE.md` |
| 查同步機制（EventBus/Realtime/FCM） | `planning/SYNC_ARCHITECTURE_SPEC.md` |
| 查訓練權限邏輯 | `planning/archived/TRAINING_PERMISSION_MATRIX.md` |
| 部署應用 | `DEPLOYMENT_GUIDE.md` |
| 配置推播通知 | `FCM_SETUP_GUIDE.md` |
| 使用 Python 工具 | `scripts/README.md` |
