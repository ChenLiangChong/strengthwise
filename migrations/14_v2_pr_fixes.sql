-- ============================================================================
-- StrengthWise Migration: 14_v2_pr_fixes.sql
-- ============================================================================
-- 合併自: 022_fix_pr_trigger_body_part.sql, 023_fix_updated_at.sql
-- 版本: v2.8
-- 日期: 2026-01-04
-- ============================================================================
-- 
-- 包含:
-- 1. update_personal_records() 觸發器修復（自動獲取 body_part）
-- 2. PR 回滾邏輯（取消勾選 set 時正確回滾 PR）
-- 3. 修復被錯誤更新的 updated_at
-- ============================================================================
...
-- 
-- ⭐ 核心邏輯變更：
-- - 自動從 exercises 或 custom_exercises 表獲取 body_part
-- - 取消勾選 set 時會正確回滾 PR
-- - 重新掃描所有歷史完成 sets，計算真正最大值
-- - 如果某動作所有 sets 都取消，則刪除該動作的 PR
...
-- ============================================================================
-- PART 3: 修復被錯誤更新的 updated_at (來自 023)
-- ============================================================================
...
DO $$ BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '====================================================================';
  RAISE NOTICE '✅ Migration 14 完成：PR 觸發器修復';
  RAISE NOTICE '====================================================================';
  RAISE NOTICE '';
  RAISE NOTICE '修復內容：';
  RAISE NOTICE '  ✅ update_personal_records() 自動獲取 body_part';
  RAISE NOTICE '  ✅ 優先從 exercises 表，其次從 custom_exercises 表';
  RAISE NOTICE '  ✅ 現有 PR 已重新初始化';
  RAISE NOTICE '  ✅ 修復被錯誤更新的 updated_at';
  RAISE NOTICE '';
  RAISE NOTICE '⭐ 核心邏輯變更：';
  RAISE NOTICE '  - 只統計 set.completed = true 的組';
  RAISE NOTICE '  - 取消勾選 set 時會正確回滾 PR';
  RAISE NOTICE '  - 重新掃描所有歷史完成 sets';
  RAISE NOTICE '  - 某動作所有 sets 取消則刪除該 PR';
  RAISE NOTICE '';
END $$;
