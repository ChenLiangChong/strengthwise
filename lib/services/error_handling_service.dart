...
class ErrorHandlingService {
  ...
  void handleError(BuildContext context, dynamic error,
      {String? customMessage, String? type, bool showNotification = true}) {
    _ensureInitialized();
    ...
    if (showNotification && context.mounted) {
      NotificationUtils.showError(
        context,
        customMessage ?? '操作執行失敗: ${_getUserFriendlyMessage(error)}',
  ...
  /// 處理載入數據時的錯誤
  void handleLoadingError(BuildContext context, dynamic error,
    ...
    if (context.mounted) {
      NotificationUtils.showError(
        context,
        customMessage ?? '載入數據失敗: ${_getUserFriendlyMessage(error)}',
        duration: _notificationDuration,
      );
    }
  }
  ...
  /// 處理數據保存時的錯誤
  void handleSavingError(BuildContext context, dynamic error,
      {String? customMessage, VoidCallback? onRetry}) {
    _ensureInitialized();
    ...
    if (context.mounted) {
      NotificationUtils.showError(
        context,
        customMessage ?? '保存數據失敗: ${_getUserFriendlyMessage(error)}',
        duration: _notificationDuration,
      );
      // TODO: 未來可以在 NotificationUtils 中添加 action 支持
      // 目前 onRetry 功能暫時不可用，需要擴展 NotificationUtils
    }
  }
  ...
  String _getUserFriendlyMessage(dynamic error) {
    ...
    // Supabase 相關錯誤
    if (errorStr.contains('supabase') || errorStr.contains('postgres')) {
