  ...
  void _setupAuthStateListener() {
    ...
    _authStateSubscription =
      ...
      if (user != null) {
        ...
        // 執行資料遷移（如果需要）
        try {
          final migrationService = serviceLocator<UserMigrationService>();
          final needsMigration =
              await migrationService.needsMigration(user.uid);

          if (needsMigration) {
            _logDebug('檢測到需要遷移的用戶資料，開始遷移: ${user.uid}');
            final success = await migrationService.migrateUserData(user.uid);

            if (success) {
              _logDebug('用戶資料遷移成功: ${user.uid}');
            } else {
              _logDebug('用戶資料遷移失敗或已遷移: ${user.uid}');
            }
          }
        } catch (e) {
          _logError('執行用戶資料遷移時發生錯誤: $e');
          // 不影響登入流程，繼續執行
        }
      } else {
        _logDebug('用戶已登出');
  ...
  Future<Map<String, dynamic>?> signInWithGoogle() async {
    ...
    try {
      ...
      if (errorStr.contains('12500') ||
        ...
        _logError(
            'Google登入錯誤: 在模擬器上 Google 登入可能無法正常工作。請使用真實設備或使用電子郵件登入。錯誤詳情: $e');
