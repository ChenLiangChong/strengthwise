...
class StatisticsService implements IStatisticsService {
  ...
  Future<List<_UnifiedWorkoutData>> _getCompletedWorkouts(
    ...
    try {
      ...
      for (var doc in snapshot.docs) {
        try {
          final data = doc.data();
          final workout = _parseWorkoutData(doc.id, data);
          
          // 過濾日期範圍
          if (workout.completedTime.isAfter(startDate) && 
              workout.completedTime.isBefore(endDate.add(Duration(days: 1)))) {
            workouts.add(workout);
          }
        } catch (e) {
          _errorService.logError('解析訓練記錄失敗 (${doc.id}): $e', type: 'StatisticsServiceError');
        }
      }
  ...
  _UnifiedWorkoutData _parseWorkoutData(String id, Map<String, dynamic> data) {
    ...
    return _UnifiedWorkoutData(
      ...
      completedTime: (data['updatedAt'] as Timestamp?)?.toDate() ?? DateTime.now(),
      exercises: exercises,
    );
  }
  ...
  Future<List<_UnifiedWorkoutData>> _getAllCompletedWorkouts(String userId) async {
    ...
    for (var doc in snapshot.docs) {
      try {
        workouts.add(_parseWorkoutData(doc.id, doc.data()));
      } catch (e) {
        _errorService.logError('解析訓練記錄失敗 (${doc.id}): $e', type: 'StatisticsServiceError');
      }
    }
