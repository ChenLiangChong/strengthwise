...
class AuthController extends ChangeNotifier implements IAuthController {
  ...
  void _handleError(String errorMsg, {dynamic originalError}) {
    _isLoading = false;
    _errorMessage =
        AuthErrorHandler.getUserFriendlyError(errorMsg, originalError);
  ...
  Future<bool> signInWithGoogle() async {
    ...
    try {
      ...
      // Web 平台：OAuth 流程會重定向到 Google 頁面
      // 返回 false 讓 UI 不跳轉，等待用戶從 Google 返回後由 Auth State 監聽器處理
      if (kIsWeb) {
        _isLoading = false;
        _errorMessage = null; // 清除錯誤訊息，這不是錯誤
        notifyListeners();
        return false; // 不跳轉，等待 OAuth 重定向完成
      }

      // 非 Web 平台：null 表示用戶取消登入
      ...
      if (errorStr.contains('模擬器') || errorStr.contains('真實設備')) {
        _handleError("Google 登入在模擬器上不可用。請使用真實設備測試，或使用電子郵件登入功能。",
            originalError: e);
      } else {
